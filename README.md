...
| ▶️ Запуск | локально за 4 команды (см. ниже) — **без каких-либо ключей** |
| 📚 Swagger | `http://localhost:8000/docs` (после запуска) |

> Сервис запускается и **полностью работает без ключей**: при отсутствии
> `ANTHROPIC_API_KEY` AI-анализ уходит в graceful fallback (это отдельное
> требование ТЗ — «если AI недоступен, сервис продолжает работать»), email без
> `RESEND_API_KEY` пишется в лог. Ключи нужны только если хотите увидеть «живые»
> вызовы Claude / Resend — для проверки задания они не обязательны.
...
- **Проверка:** все эндпоинты прогнаны вживую (`curl`), 9 тестов pytest зелёные,
...
> Для проверки задания деплой не требуется — достаточно локального запуска выше.
> Этот раздел показывает, что сервис serverless-ready: репозиторий уже содержит
> `vercel.json` и `api/index.py`, и разворачивается одной командой.
